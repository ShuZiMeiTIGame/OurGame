...
>>>>>>> origin/master
	...
	void Joint1(Vec2 p, Vec2 s, PhysicsWorld*world);//小车	//p 中点 s长方形的长宽 

	void Joint3(Vec2 p, std::vector<Vec2>* pos, float w, float h, PhysicsWorld*world);//跷跷板 w宽 h高
	void Joint4(Vec2 p1, Vec2 p2, Vec2 s1, Vec2 s2, int c1, int c2, PhysicsWorld*world);//两个长方形的弹簧吧 可能/c密度
	void Joint6(Vec2 p, Vec2 o, PhysicsWorld*world, int r);//旋转的长方形//p 中点 s长方形的长宽 r旋转圆半径
